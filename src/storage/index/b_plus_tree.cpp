...
INDEX_TEMPLATE_ARGUMENTS
template <typename N>
auto BPLUSTREE_TYPE::CoalesceOrRedistribute(N *node, Transaction *transaction, bool is_root_page_id_latched) -> bool {
  if (node->IsRootPage()) {
...
auto BPLUSTREE_TYPE::Coalesce(N **neighbor_node, N **node,
                              ...
                              Transaction *transaction, bool is_root_page_id_latched) -> bool {
...
void BPLUSTREE_TYPE::Redistribute(N *neighbor_node, N *node, BPlusTreeInternalPage<KeyType, page_id_t, KeyComparator> *parent,
                                  int index, bool is_root_page_id_latched) {
  if (is_root_page_id_latched) {
    root_page_id_latch.unlock();
  }
 ...
 */
INDEX_TEMPLATE_ARGUMENTS
auto BPLUSTREE_TYPE::AdjustRoot(BPlusTreePage *old_root_node, bool is_root_page_id_latched) -> bool {
  // case 1: when you delete the last element in root page, but root page still
  // has one last child
