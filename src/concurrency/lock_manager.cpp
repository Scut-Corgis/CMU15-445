...
bool LockManager::LockShared(Transaction *txn, const RID &rid) {
  ...
  auto lock_request = LockRequest(txn->GetTransactionId(), LockManager::LockMode::SHARED);
...
bool LockManager::LockExclusive(Transaction *txn, const RID &rid) {
  ...
  std::unique_lock<std::mutex> queue_latch(lock_request_queue.latch_);
  auto lock_request = LockRequest(txn->GetTransactionId(), LockManager::LockMode::EXCLUSIVE);
  lock_request_queue.request_queue_.push_back(lock_request);
...
bool LockManager::ProcessDFSTree(txn_id_t *txn_id, std::stack<txn_id_t> *stack,
  ...
  std::pair<txn_id_t, LockManager::VisitedType> pairTemp = {stack->top(), LockManager::VisitedType::VISITED};
  visited->insert(pairTemp);
  stack->pop();
